...
async def add_to_cart(user_id: int, product_id: int, qty: int = 1):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # спочатку спробуємо збільшити qty
        cur = await db.execute(
...
async def update_cart_qty(user_id: int, product_id: int, qty: int):
    """Встановити qty точним значенням; якщо qty <=0 - видалити рядок."""
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id FROM carts WHERE user_id=?", (user_id,))
        ...
        if qty <= 0:
            ...
            await db.execute("UPDATE cart_items SET qty=? WHERE cart_id=? AND product_id=?", (qty, cart_id, product_id))
        await db.commit()
