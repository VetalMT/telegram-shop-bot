...
async def admin_add_product_start(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    await state.set_state(AddProductFSM.name.state)
...
async def admin_add_product_name(message: types.Message, state: FSMContext):
    ...
    await state.set_state(AddProductFSM.description.state)
...
async def admin_add_product_description(message: types.Message, state: FSMContext):
    ...
    await state.set_state(AddProductFSM.price.state)
...
async def admin_add_product_price(message: types.Message, state: FSMContext):
    ...
    await state.set_state(AddProductFSM.photo.state)
...
async def admin_add_product_photo(message: types.Message, state: FSMContext):
    if not message.photo:
        await message.answer("Надішліть фото або /skip")
        return
...
async def _confirm_product(message: types.Message, state: FSMContext):
    ...
    await state.set_state(AddProductFSM.confirm.state)
...
async def admin_add_product_confirm(message: types.Message, state: FSMContext):
    ...
    if text != "+":
        await message.answer("Використайте '+' для збереження або '-' для скасування.")
        return
...
def register_admin_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(admin_add_product_start, Text(equals="➕ Додати товар"))
    dp.register_message_handler(admin_add_product_name, state=AddProductFSM.name)
    dp.register_message_handler(admin_add_product_description, state=AddProductFSM.description)
    dp.register_message_handler(admin_add_product_price, state=AddProductFSM.price)
    dp.register_message_handler(admin_add_product_skip_photo, Command("skip"), state=AddProductFSM.photo)
    dp.register_message_handler(admin_add_product_photo, content_types=types.ContentType.PHOTO, state=AddProductFSM.photo)
    dp.register_message_handler(admin_add_product_confirm, state=AddProductFSM.confirm)
    dp.register_message_handler(admin_view_products, Text(equals="📦 Переглянути товари"))
    dp.register_message_handler(admin_delete_prompt, Text(equals="❌ Видалити товар"))
    dp.register_message_handler(admin_delete_by_id, lambda m: m.text and m.text.isdigit())
