...
from db import get_products, add_to_cart, get_cart, remove_from_cart, clear_cart, create_order, update_cart_qty
...
async def cb_cart_remove(callback: types.CallbackQuery):
    ...
    if not item:
        return
    # якщо кількість 1 => видалити, інакше зменшити на 1
    if item["qty"] <= 1:
        await remove_from_cart(callback.from_user.id, product_id)
    else:
        await update_cart_qty(callback.from_user.id, product_id, item["qty"] - 1)
    await show_cart(callback.from_user.id, cq=callback)
